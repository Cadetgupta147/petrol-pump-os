...
- [ ] **Editing/deleting a bill does not recompute or reverse its loyalty points (§6.3).** `bills.service.ts` `update()`/`remove()` carry `KNOWN GAP` comments: a bill edited after points were credited, or soft-deleted, still leaves its original points on the customer's balance. Fix: write compensating `LoyaltyTransaction` entries on edit/delete.
- [ ] **`loyaltyWarning` (bill created despite no LoyaltyConfig) has no UI surface yet** on either web-portal or dsm-app — the API returns the warning on `POST /bills` but nothing displays it. See the §8A.3 pattern for how other silent-failure risks got a banner; apply the same idea here.
