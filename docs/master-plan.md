...
18. [Go-Live Readiness Checklist](#18-go-live-readiness-checklist)
19. [A–Z Quick Reference Index](#19-az-quick-reference-index)
...
#### 3.3.1 Nozzle Master (Settings) — resolved, no longer an open item

*(Researched against Softgum, Petro360, PumpManager, and similar Indian petrol-pump ERPs before implementation — see the "Researched but deliberately deferred" note below for what that research surfaced beyond this slice's scope.)*
...
- [ ] **17.1** — Decide default loyalty earning basis (rupee vs. litre) and default rate
- [ ] **17.2** — Decide redemption type allowed at launch: cash-only, gift-only, or both
- [ ] **17.3** — Source your first gift-catalog SKUs and confirm you can physically stock them at the counter
- [ ] **17.4** — Decide OCR provider based on cost at your expected invoice volume
- [ ] **17.5** — Decide Tally export approach: XML file export (simple) vs. direct API push (Phase 2+)
- [ ] **17.6** — Decide offline-sync behavior for DSM app in low-connectivity areas
- [ ] **17.7** — Decide receipt printer hardware model (Bluetooth thermal printer compatibility)
- [ ] **17.8** — Decide WhatsApp Business API provider (Gupshup, Interakt, or direct Meta API)
- [ ] **17.9** — Confirm legal entity structure before any future cross-business loyalty (e.g. a second business sharing the same points program)
- [ ] **17.10** — Agree on the Person A / Person B split (Section 16.1) explicitly before writing code, to avoid overlapping work
- [ ] **17.11** — **Data privacy (DPDP Act):** customer phone numbers, vehicle numbers, and KYC-lite loyalty profiles (§6.1) are personal data under India's Digital Personal Data Protection Act. Before Phase 3 collects real customer data, confirm you have basic consent capture at signup and a way to delete a customer's data on request — doesn't need to be elaborate at your scale, but it does need to exist.
- [x] **17.12** — **Informal walk-in credit tabs — RESOLVED, see §3.4A.** Walk-in credit doesn't require full customer registration (quick-add + `informal` flag, shown yellow in UI). Credit limit enforcement is a dealer-configurable setting (`NOTIFY` default — bill still goes through, dealer gets an alert with an optional one-tap payment reminder; `BLOCK` available for dealers who want a hard stop), with a dealer-set default informal credit limit so exposure isn't fully unbounded.
- [ ] **17.13** — **Editing/deleting a bill does not recompute or reverse its loyalty points (§6.3).** `bills.service.ts` `update()`/`remove()` carry `KNOWN GAP` comments: a bill edited after points were credited, or soft-deleted, still leaves its original points on the customer's balance. Fix: write compensating `LoyaltyTransaction` entries on edit/delete.
- [ ] **17.14** — **`loyaltyWarning` (bill created despite no LoyaltyConfig) has no UI surface yet** on either web-portal or dsm-app — the API returns the warning on `POST /bills` but nothing displays it. See the §8A.3 pattern for how other silent-failure risks got a banner; apply the same idea here.
- [ ] **17.15** — **Web portal customer edit form seeds from stale list data (§3.4).** `CustomerFormModal` in edit mode is seeded from the row object already in memory (from `GET /customers`), not a fresh `GET /customers/:id` fetch. Since the form submits all four editable fields on save (not a diff), a stale row can silently overwrite a field someone else changed in the meantime — a lost-update, not just a display staleness issue. Accepted as-is at current team size (solo; only Owner/Accountant can reach this endpoint, so the realistic collision window is one person in two tabs). **Fix trigger: add fetch-on-open once the Person A / Person B split (§16.1) is actually active** — two people editing concurrently is the point this stops being a theoretical risk. Fetch-on-open narrows the window but doesn't fully close it without optimistic concurrency (e.g. an `updatedAt`/version check on `PATCH /customers/:id`); that's over-engineering until team size or edit volume justifies it.
- [ ] **17.16** — **Customer app bill history shows product type + litres, not a nozzle number (§5, §14).** `Bill` has no nozzle field, so `GET /customer-portal/bills` (`apps/backend/src/customer-portal`) can't return one — the customer-app history/home screens show date, product type, litres, amount, and points earned instead of the "Nozzle 2" line in the §14 home mockup. Add a nozzle field (and the `MeterReading`/`Bill` linkage to populate it) only if customers actually ask "which pump did I use" — not worth a schema change for a hypothetical.
- [ ] **17.17** — **"Pay Now" (§5) has never been fully specced, not just "gateway undecided."** Beyond picking a provider (PhonePe/Paytm vs. a checkout gateway — see the open item above and CLAUDE.md), nobody has specified the actual flow: UPI deep-link (pre-filled intent to the customer's own UPI app, no gateway needed) vs. a full hosted checkout (needs Razorpay/Cashfree-style integration, webhook reconciliation, refund handling), or how a successful payment reconciles against `Customer` outstanding balance (a new `Payment` row presumably, but written by what — a webhook? a client callback? both, for idempotency?). The customer app currently ships a "coming soon" notice on this button rather than building against a guessed contract. **Needs its own spec subsection before this is handed to backend-agent/mobile-agent as an implementation task.**
- [ ] **17.18** — **QR-card self-service linking has no backend support (§6.1, §14 home mockup).** The home-screen mockup shows a "Link your physical QR loyalty card" banner, but there's no endpoint for a customer to associate their logged-in app account with a `qrMemberId` — today, cards are only issued/printed by staff (`POST /customers` allocates the id; `GET /customers/:id/qr` renders it), and a customer's own JWT (`customerId`) is already tied 1:1 to the `Customer` row that owns a `qrMemberId`, so "linking" really means "confirm this physical card's printed id matches the account you're logged into," not a free-floating association. Needs a linking endpoint (customer types/scans the member id printed on their card; server confirms it matches their own `req.user.customerId`, rejects if it belongs to someone else) before this banner can be built — flagged in the same spirit as the redemption endpoint's "never trust a client-supplied customerId" rule (CLAUDE.md).
- [ ] **17.19** — **`DENSITY_RANGE_BY_PRODUCT` (§7.3) is a placeholder, not a real compliance threshold.** `apps/backend/src/density-logs/density-logs.service.ts` flags a density reading as out-of-range using hardcoded bands (`petrol: 0.72–0.775 g/mL`, `diesel: 0.82–0.87 g/mL`) chosen as reasonable-looking defaults, not sourced from IOCL/BPCL/HPCL quality-control specs. Same situation as `DIP_VARIANCE_TOLERANCE_LITRES` (§7.2) — an undocumented-in-the-plan tolerance the code had to invent something for. Fine for exercising the flagging logic today, but since §7.3's whole reason for existing is OMC audit compliance, this must be replaced with your OMC's actual published acceptable density/PPM ranges (and ideally made dealer-configurable per §7.2's tolerance, since different OMCs/products may specify different bands) before `flagged` on a `DensityLog` is relied on for a real audit. A product not in the map is never flagged — silently trusting an unlisted product is a bigger compliance risk than the placeholder ranges themselves, so extending the map's product coverage matters as much as correcting its numbers.
- [x] **17.20** — **QR-scan-after-credit-picker could silently reattribute a bill's credit line to the wrong customer — RESOLVED as of this commit.** In `apps/dsm-app/src/screens/NewBillScreen.tsx` (Section 4/5A), the screen has one "customerId slot" per bill, filled either by `CreditCustomerPicker` (manual pick, for a `CREDIT` payment line) or by a QR scan. Scanning a *different* customer's QR after a `CREDIT` line was already added for the manually-picked one silently overwrote the slot — `creditLineNeedsCustomer`'s existing guard only checked "is *some* customer attached," not "is it the *same* one the CREDIT line was opened against" — so the bill would save attributing one customer's credit purchase to another, with no warning. (The on-screen "Credit customer: ..." label was suspected to compound this by staying stale, but on inspection it was already correctly cleared everywhere `creditCustomerId`/`creditQuickAdd` were cleared — the live bug was purely the missing conflict check.) A live-DB audit (2026-07-20, read-only, via `CustomersService.ledger()`'s balance derivation) found **zero** DSM-app bills ever created with a `CREDIT` payment line, so no bad data resulted — this was caught before any real usage. Fixed via `hasCreditCustomerConflict()` (`apps/dsm-app/src/screens/creditCustomerConflict.ts`): a scan that would reattribute an existing `CREDIT` line to a different customer is now blocked with a visible on-screen error instead of silently applied, and the DSM must remove the existing `CREDIT` line first before a different customer can be attached.
...
## 18. Go-Live Readiness Checklist

Before this system takes real customer money or real customer data, work through all three parts below — not just the code, the process around it.

### 18.1 Hard blockers — do not take real customer money/data until resolved

- [ ] **UPI webhook signature scheme is still a placeholder (§8A.3).** Confirm no real PhonePe/Paytm Business webhook is registered against this endpoint yet, and that a real provider + signature scheme is chosen before relying on automated UPI capture.
- [ ] **Bill edit/delete does not reverse loyalty points earned on that bill (§17.13).** Needs to actually be fixed before real customer point balances depend on it being correct.
- [ ] **Confirm the React 18/19 workspace version conflict (flagged earlier, fix prompted) was actually resolved** — check `npm run build` at the repo root.
- [ ] **Confirm production hosting is in place.** Supabase's free tier pauses after 7 days of inactivity and has no automated backups; not appropriate for a system holding real customer credit balances and loyalty points.
- [ ] **Confirm the DPDP Act item (§17.11) was actually implemented, not just logged** — consent capture at signup, and a way to delete a customer's data on request.

### 18.2 Known gaps — safe to launch with, as long as the affected feature isn't used

- [ ] **GST report is not real tax-filing-ready (§17.22).** Get sign-off from an actual accountant before relying on it for filing, not just internal use.
- [ ] **Staff wage/advance tracking has no schema support yet (§17.23).**
- [ ] **Customer app "Pay Now" is a placeholder** — no real payment flow (§17.17).
- [ ] **QR self-service card linking is not built (§17.18).**
- [ ] **Density/PPM range thresholds are placeholders, not real OMC values (§17.19).**
- [ ] **OCR parser has known edge cases** (documented false-positive fix already applied for one case; others may exist).

### 18.3 The step that matters most — dogfooding, not more code

- [ ] **Run the new system in parallel with the current manual process for 2–4 weeks before fully switching over (§16.5).** Don't discard the paper/Excel process until at least one full month has gone through cleanly. **Not started yet as of this checklist's creation (2026-07-21).**

---

## 19. A–Z Quick Reference Index
