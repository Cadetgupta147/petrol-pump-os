...
// mocked PrismaService (unlike every *.spec.ts under src/, including the
// OTHER files also named *.integration.spec.ts there, which mock Prisma at
// the module-wiring level and stay fast/network-free). A mocked `tx` can't
// exercise real Postgres row-locking/blocking behavior, which is exactly
// what's under test here — proving StaffManagementService.update()'s
// last-Owner guard (see that method's row-lock comment) closes a real race
// between two genuinely concurrent transactions, not just passes a
// single-sequential-call unit test.
//
// Because of that, this file lives OUTSIDE src/ (jest's default config in
// package.json has rootDir: "src", so this directory is invisible to it) and
// runs via its own command instead of being swept up by plain `npm test`:
//
//   npm run test:integration
//
// (see jest.integration.config.js and the "test:integration" script in
// package.json). This needs the real DATABASE_URL from the repo-root .env
// (the same Supabase dev DB `npm run start:dev` connects to) to be reachable
// — there is no local/Docker DB for this project (see CLAUDE.md) — so it's
// kept out of the default `npm test` run to avoid making every routine test
// run (and CI) depend on external DB reachability and pay ~30s of real
// network round trips for a single race-condition check.
//
// This test creates its own Pump/StaffAccount/Staff rows and deletes exactly
// those rows in `afterAll` (see that hook below) — it does not touch any
// other data in the dev DB and cleans up after itself even if an assertion
...
//
// __dirname here is apps/backend/test/integration — four levels up reaches
// the repo root, same depth as when this spec lived under
// apps/backend/src/staff-management (src/staff-management -> src -> backend
// -> apps -> root is also four), so this path did not need to change when
// the file moved.
...
// row-lock comment on the last-Owner guard): two concurrent requests
...
describe('Last-Owner guard — closes the race under real concurrent transactions (integration)', () => {
  ...
  // here rather than two separate PrismaClients — confirmed empirically (no
  // maxWait/timeout flakiness observed across repeated runs).
  ...
  afterAll(async () => {
    ...
    // leak a connection and/or orphan rows in the shared dev DB. This runs
    // even if the test body's assertions above failed — afterAll always
    // runs regardless of the preceding test's outcome.
    try {
      if (staff1Id) {
