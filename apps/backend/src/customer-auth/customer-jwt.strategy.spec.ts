...
describe('CustomerJwtStrategy', () => {
  ...
  it('maps a valid payload to { customerId, phone } when tokenVersion matches the DB value', async () => {
    prisma.customer.findUnique.mockResolvedValue({ tokenVersion: 3 });
    ...
    expect(result).toEqual({ customerId: 'customer-1', phone: '9990000001' });
  ...
  // claim, see prisma/schema.prisma's Customer.tokenVersion comment.
  it('rejects a payload whose tokenVersion no longer matches the DB value', async () => {
    prisma.customer.findUnique.mockResolvedValue({ tokenVersion: 4 }); // bumped since token was issued
    const strategy = new CustomerJwtStrategy(prisma as unknown as PrismaService);
